...
// event PrintTime(string str, )
    ...
    event RequestRedeemPrintLen(string str, uint len);
    
    
    function CreateRedeem(
        RequestRedeemInput memory redInput
    ) public returns(Verifier.SigmaProof memory, Verifier.SigmaAuxiliaries memory, MixSC.ProcessRedeemInput memory) {
        // uint256 l;//标识是哪个cesc
        // uint256 m;
        // uint256 n = 2;
        ...
        auxRedeem1.token = auxRedeem1.f_base.mul(redInput.td);
        {
            ...
            emit RequestRedeemPrintLen("user.sol create Redeem 198 the len is :", len);
            auxRedeem2.N = len;
        ...
        // emit RequestRedeemPrintLen("user.sol create Redeem 216 the 二进制长度 is :", auxRedeem1.m);
        // emit RequestRedeemPrint("user.sol create Redeem 216 is good!");
        // //emit RequestRedeemAuxRedeem1(auxRedeem1);
       //require(false, "user.sol 219 is OK , quit temporally!!!");
        ...
        emit RequestRedeemPrint("user.sol create Redeem 233 is good!");
        ...
        emit RequestRedeemPrint("user.sol create Redeem 241 is good!");
       //require(false, "user.sol 243 is OK , quit temporally!!!");

        //第二步，对密文格式进行验证，生成pf_enc（即rps）
        RedeemProofStep2 memory rps;
        {
            auxRedeem1.r = redInput.k.add(redInput.td).mod();
  ...
  //require(false, "user.sol 260 is OK , quit temporally!!!");
        ...
        // }
        for (uint256 i = 0; i < auxRedeem2.N; i++) {
            auxRedeem2.clist1[i] = cred.X.add(mixSC.getEscPoolItemByIndex(i).cesc.X.add(mixSC.getEscPoolItemByIndex(i).mu).neg());
            auxRedeem2.clist2[i] = cred.Y.add(mixSC.getEscPoolItemByIndex(i).cesc.Y.neg());
        }
//require(false, "user.sol 269 is OK , quit temporally!!!");
    ...
    emit RequestRedeemPrint("step2 near 356 is OK");
        // //生成A,C,D
            ...
            auxR1Part1.group_elements = new BN128.G1Point[](
                auxRedeem2.Gk1.length + auxRedeem2.Gk2.length + 4
            );
            auxR1Part1.group_elements[0] = auxProcessRed.A;
            auxR1Part1.group_elements[1] = auxProcessRed.B;
            auxR1Part1.group_elements[2] = auxProcessRed.C;
            auxR1Part1.group_elements[3] = auxProcessRed.D;
            for (uint256 i = 0; i < auxRedeem2.Gk1.length; i++) {
                auxR1Part1.group_elements[i + 4] = auxRedeem2.Gk1[i];
            }
            for (uint256 i = 0; i < auxRedeem2.Gk2.length; i++) {
                auxR1Part1.group_elements[auxRedeem2.Gk1.length + i + 4] = auxRedeem2.Gk2[i];
            }
            auxR1Part1.challenge_x = Primitives.generateChallenge(auxR1Part1.group_elements);
            ...
            //生成f,zA,zC
            {
                ...
                auxPart2Input.challenge_x = auxR1Part1.challenge_x;
            ...
            for(uint256 i = 0; i < auxRedeem1.m; i++) {
                auxProcessRed.tmpz = auxProcessRed.tmpz.add( auxRedeem2.rouk[i].mul(auxR1Part1.challenge_x.modExp(i)).mod() );
            }
            auxProcessRed.z = auxRedeem1.r.mul(auxR1Part1.challenge_x.modExp(auxRedeem1.m)).sub(auxProcessRed.tmpz);
